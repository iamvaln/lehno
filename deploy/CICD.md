...
| `CERT_RESOLVER` | quel résolveur ACME demande les certificats. Absente, `le` — le défi TLS-ALPN, celui qui sert depuis le début. À passer à `le-dns` **avant** de mettre un nom derrière le proxy de Cloudflare : celui-ci termine le TLS lui-même, donc TLS-ALPN ne parvient jamais à Traefik, et le renouvellement échoue **soixante jours plus tard**, en silence, sur un domaine qui servait très bien. Le résolveur `le-dns` vit dans le Traefik partagé (`gabee/deploy/proxy`) et demande `CF_DNS_API_TOKEN` là-bas |
