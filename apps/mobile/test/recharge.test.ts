...
describe("les paliers", () => {
  ...
  it("à défaut de remise, le premier", () => {
    const grille = [palier(3, 30, 2200, 3, null), palier(1, 5, 500, 1, null)];
    expect(palierParDefaut(grille)).toBe(uuid(1));
  });
  ...
  it("aucun palier n'invente aucun montant", () => {
    expect(palierParDefaut([])).toBeNull();
  });
});
...
describe("le récapitulatif", () => {
  ...
  it("prend le total servi, jamais montant + frais", () => {
    const paye = recapDuPaiement(apercu({ amount: 1000, fee: 20, amountToSend: 1000, expectedOnAccount: 980 }));
  ...
  it("porte le total du barème où le client supporte les frais", () => {
    expect(recapDuPaiement(apercu()).total).toBe(1020);
  });

  it("tait des frais nuls plutôt que d'afficher zéro", () => {
    expect(recapDuPaiement(apercu({ fee: 0 })).frais).toBeNull();
  });

  /* LE CHAMP SERVI, sans le recalculer. Le mobile ne vérifie pas le serveur :
     on ne fait confiance qu'à lui, et un téléphone n'a pas à porter un
     traitement qu'on peut lui épargner. Que `bonus_percent` soit aujourd'hui
  ...
  it("rend les crédits et la devise servis", () => {
    const paye = recapDuPaiement(apercu({ credits: 30, currency: "EUR" }));
