...
export function parametresDuCarnet(tri: Tri, offset: number): string {
  /* PAS DE `includeSelf` — le serveur exclut la fiche de soi sans condition
     depuis le 21 septembre, et `total` suit ce filtre de son côté. Le
     paramètre a existé le temps que ce ne soit pas encore vrai ; il a disparu
     du contrat en même temps que le besoin. */
