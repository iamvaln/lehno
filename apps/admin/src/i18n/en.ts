...
export const en: typeof fr = {
  ...
  studioTextes: {
    ...
    orientations: {
      ...
      consigneAide: "What the model receives for this angle. Changing it makes a new version: a fresh trial is needed before publishing.",
      libelle: "Label",
      libelleAide: "What the client reads in the list. Both languages are required.",
      description: "Description",
      avertissement: "Warning",
      facultatifAide: "Optional. Both languages together, or neither — one side alone is refused.",
      avertissementAide: "Optional. What warns before picking this angle. Both languages together, or neither.",
      texteIncomplet: "A text is filled on one side only: {lesquelles}. Both languages go together.",
    },
  ...
  clientsApi: {
    titre: "API clients",
    sous: "Who may call, and under which name.",
    portee: "A distributed app carries its key in its bundle: whoever unpacks it has it. This is not a security boundary — it is a REVOCABLE identifier, and cutting a client cuts a whole application without touching the others.",
    col: { libelle: "Client", identifiant: "Identifier", type: "Platform", env: "Environment", etat: "State", tournee: "Key rotated on" },
    types: { mobile_ios: "iOS", mobile_android: "Android", web: "Web" },
    envs: { dev: "Development", staging: "Staging", prod: "Production" },
    etats: { ouvert: "Open", coupe: "Cut" },
    jamaisTournee: "Never",
    ouvrir: "Open a client",
    tourner: "Rotate the key",
    couper: "Cut",
    rouvrir: "Reopen",
    champs: { libelle: "Client name", type: "Platform", env: "Environment" },
    cle: {
      titre: "This client's key",
      unique: "It will never be shown again. Copy it now.",
      perdue: "A lost key cannot be recovered: it is replaced by rotating the client's key, which cuts the old one.",
      copiee: "I have copied it",
    },
    dialogueOuvrir: {
      titre: "Open an API client",
      consequence: "A key will be generated and shown ONCE. The identifier, however, can always be read again.",
      motifs: ["New contract", "Staging environment", "Load test"],
    },
    dialogueTourner: {
      titre: "Rotate {client}'s key",
      consequence: "The old key stops being accepted immediately, and devices still using it will be refused. The identifier does not change: the figures stay comparable over time.",
      motifs: ["Access compromised", "Routine check"],
    },
    dialogueCouper: {
      titre: "Cut {client}",
      consequence: "The whole application stops being served, on every device at once. Nothing is deleted: rows already recorded keep their reference, and the client can be reopened.",
      motifs: ["Access compromised", "Fixing an error"],
    },
    dialogueRouvrir: {
      titre: "Reopen {client}",
      consequence: "The application is served again, with the same key as before the cut.",
      motifs: ["Fixing an error", "Routine check"],
    },
    vide: {
      titre: "No client",
      texte: "Open pairs appear here, with their state and the date of their last rotation.",
    },
  },
  versions: {
    titre: "App versions",
    sous: "What we agree to serve, and what we turn away.",
    portee: "Forcing an update takes every device below that build out of service, at once. We do it for a breaking change or a mandatory security fix — and then everyone has to move, ten devices or ten thousand. The account count tells you the scale to prepare for, never whether to do it.",
    col: {
      plateforme: "Platform", version: "Version", build: "Build", etat: "State",
      vus: "Accounts seen", publiee: "Published",
    },
    plateformes: { mobile_ios: "iOS", mobile_android: "Android", web: "Web" },
    etats: { servie: "Served", forcee: "Update forced", declassee: "Retired" },
    toutes: "All platforms",
    vus: "{n} in 30 d",
    ampleur: "{n} accounts were seen on this build in thirty days: enough to warn support and write the announcement.",
    enregistrer: "Register a version",
    forcer: "Force the update",
    liberer: "Stop forcing",
    declasser: "Retire",
    champs: {
      plateforme: "Platform",
      version: "Version number",
      build: "Build",
      buildAide: "This is what identifies the version, not its number: two builds can carry the same one.",
      lien: "Store link (optional)",
      lienInvalide: "A full address, starting with https://",
    },
    dialogueEnregistrer: {
      titre: "Register a version",
      consequence: "The version becomes served. It forces nothing: forcing is decided afterwards, on the row, with its own reason.",
      motifs: ["New release", "Routine check", "Fixing an error"],
    },
    dialogueForcer: {
      titre: "Force the update from {version} (build {build})",
      consequence: "Every device below this build will be unable to call until it has updated.",
      motifs: ["Security warning", "Fixing an error", "Access compromised"],
    },
    dialogueLiberer: {
      titre: "Stop forcing from {version} (build {build})",
      consequence: "Devices below this build are served again, with nothing to do on their side.",
      motifs: ["Fixing an error", "Routine check"],
    },
    dialogueDeclasser: {
      titre: "Retire {version} (build {build})",
      consequence: "The version stops being offered. Nothing is deleted: the row stays in the registry, and the history still reads.",
      motifs: ["Routine check", "Fixing an error"],
    },
    vide: {
      titre: "No version yet",
      texte: "Registered versions appear here, newest build first.",
    },
  },
  confirmation: {
    motif: "Reason",
  ...
  modeles: {
    ...
    col: { fournisseur: "Provider", modele: "Model", capacite: "Handles", etat: "State", emplois: "Where it serves", rejets: "Rejected", entree: "Input cost", sortie: "Output cost", parImage: "Cost per image", qualite: "Quality" },
    capacites: { texte: "Text", image: "Image" },
    etats: { actif: "In service", eteint: "Off", enPanne: "Briefly unreachable" },
    sansCout: "Not priced",
    sansObjet: "—",
    qualiteAuFournisseur: "Provider decides",
    qualites: { low: "Low", medium: "Medium", high: "High" },
    tarifer: "Set the prices",
    dialogueTarif: {
      titre: "Set the prices for {modele}",
      consequence: "Prices are what turns each call into a cost. A model with no price leaves that cost at zero, and a zero in a margin calculation reads as a fact. Quality changes both what the provider produces and what it bills.",
      motifs: ["Provider price updated", "Fixing an error", "Quality changed"],
    },
  ...
  credits: {
    ...
    reglages: {
      formulaire: {
        ...
        titreTarif: "Edit this action\u2019s price",
      ...
      tarifs: {
        titre: "Action prices",
        sous: "What an action costs the person taking it, in credits. The list comes from the contract registry: you set the price, you do not open a new one.",
        vide: { titre: "No paid action", texte: "The actions the contract registry declares appear here, with their price in credits." },
        col: { libelle: "Action", code: "Code", cout: "Credits", etat: "State" },
        gratuit: "Free",
      },
      actif: "In service",
      inactif: "Withdrawn",
  ...
  studioAtelier: {
    ...
    chaine: {
      ...
      profil: "Test profile",
      voie: "Path tested",
      voies: { illustration: "Illustration", photo: "Processed photo" },
      sansPhoto: "This profile has no example photo: the photo path cannot be tested on it. One is uploaded from « Simulation profiles ».",
      modele: "Model called",
    ...
    photo: {
      titre: "The photo path",
      sous: "What the model is asked to do with the uploaded photo, and what gets turned away.",
      consigne: "Photo instruction",
      consigneAide: "It goes into the prompt along with the photo. Changing it makes a new version: a fresh trial is needed before publishing.",
      seuils: "What gets turned away on arrival",
      seuilsAide: "These three thresholds reject a photo before any call to the model. Changing them asks for no new trial.",
      coteMin: "Smallest side, in pixels",
      coteMinAide: "From 256 to 4096. Below that, upscaling invents detail — and the model renders a face that is nobody's.",
      luminositeMin: "Average brightness",
      luminositeMinAide: "From 0 to 255. A photo too dark carries nothing to draw from; brightening it would reveal noise.",
      nettetteMin: "Local contrast",
      nettetteMinAide: "From 0 to 128. This is the standard deviation of levels, an APPROXIMATE measure of sharpness — a blurred image has little local contrast. It is not a blur calculation.",
      absent: "No photo settings are published. The thresholds below are the code's and already apply; the instruction, however, does not exist — the model receives nothing about the photo.",
      consigneExigee: "The photo settings go together: without the instruction in both languages, the thresholds cannot be published.",
    },
    interne: {
