# Terms of Service

_Version 2026-08-27 · Last updated: 27 August 2026_

## 1. Purpose

These terms of use (the "Terms") govern access to and use of the Lehno service
(the "Service"), provided by **Proxia Digital** (the "Provider") through its
website and its mobile app.

By creating an account or using the Service, you confirm that you have read,
understood and accepted these Terms and the
[privacy policy](/v1/public/legal/confidentialite?lang=en).

## 2. Applicable legal framework

- Law no. 2024/017 of 23 December 2024 on the protection of personal data — **the
  governing text**.
- Law no. 2010/012 of 21 December 2010 on cybersecurity and cybercrime.
- Law no. 2023/007 of 25 July 2023 — Charter for the protection of children
  online, where a page concerns a minor.
- Regulation (EU) 2016/679 ("GDPR") for users residing in the European Union
  only, alongside Cameroonian law.

## 3. Definitions

- **User**: anyone aged 18 or over — the data-protection age of majority under
  law no. 2024/017 — who has created an account. The civil age of majority in
  Cameroon is 21.
- **Account**: the personal space attached to an email address.
- **Page**: what a User records about someone close to them.
- **Wall**: a User's public page, at the address they chose.
- **Link**: a token address giving access to a single resource — collecting
  information, leaving a birthday note, an invitation.
- **Credit**: the unit that pays for one piece of content created by the Service.

## 4. What the Service does

Lehno helps its users remember the dates that matter and what they know about the
people they love, and be ready when the day comes. It lets you:

- keep one page per person and note what you learn over the year;
- record dated occasions and be reminded of them;
- have someone fill in a page themselves, through a link;
- publish a Wall where the people around you find your wishes and can leave you a
  note;
- obtain, in exchange for credits, a portrait, gift ideas or a message written
  from what you noted.

The Service is provided "as is" and its features may change. It sends nothing on
your behalf: a message it produces is handed back to you so that you adjust it,
sign it and send it from your own inbox.

## 5. Signing up and your account

Signing up requires a valid email address. You sign in with a one-time code sent
to that address, or with a Google or Apple account you connect. The Service uses
no password.

You are responsible for access to your inbox and to the accounts you connect. The
number of accounts that can be created per device is capped, to prevent automated
sign-ups.

## 6. Credits and payment

The core of the Service is free, with no time limit: your notes, your pages, your
dates, your reminders and your Wall.

Content created for you is paid for in credits. Prices are shown in the Service
and on its website.

**We set our prices, and we may revise them at any time.** This covers the price
of a credit pack, the number of credits it holds, any discount that comes with
it, the number of credits given at sign-up, and the credit cost of each action. A
pack may also stop being offered.

**You are told beforehand.** A price change is announced to you by email
**before** it takes effect, not discovered afterwards. You therefore have time to
buy on the terms you knew.

Three things also protect you:

- **Credits you already hold keep their value.** A price change never reduces
  them, never takes them back, and never makes them expire.
- **The cost of an action is shown to you before it starts.** You decide when you
  spend, never afterwards.
- **A purchase already made keeps its terms.** The amount, the number of credits
  and the fees quoted to you remain those of your purchase, whatever tomorrow's
  prices are.

Credits are bought by mobile money or by card. **They do not expire**: they stay
yours for as long as the account exists, and you spend them in the app at your own
pace. A credit that has been spent is not refundable, the action having been
carried out. If an action fails through our fault, the credit committed is returned
to you.

**When you delete your account**, the balance of the credits you **bought** is
refunded on request, to one of your registered payment methods; the amount and the
destination are shown to you before you confirm. Two conditions protect the
operation: the method must have been registered for **more than two weeks** and must
**already have been used for a payment** on the Service. If no method meets them,
support takes over; deletion can go ahead or wait, as you prefer.

**Credits that were given are not refundable** — welcome credits, referral credits,
promotional-code credits. They were never paid for.

## 7. Your content

Your notes, your pages, your wishes and your messages are yours. You grant us
only the rights needed to host them, display them to you and — at your explicit
request — pass them to a generation processor to produce the content you ordered.
That right ends when the content or the account is deleted.

We publish nothing you write, with the single exception of what you choose to put
on your Wall.

## 8. What you undertake about other people's pages

The Service rests on what you know about people who, most of the time, have no
account. In using it, you undertake to:

- record only people with whom you have a real personal connection, and only to
  prepare an occasion that concerns them;
- keep to what serves that preparation, and leave out what does not — in
  particular any health data, opinions, beliefs, orientation, or identity
  documents;
- where the page concerns a child, record only what serves to prepare their
  occasion, and accept that their rights are exercised by their parent or legal
  representative — law no. 2023/007 of 25 July 2023 applies;
- pass on to the Provider, or handle yourself, any request for access, correction
  or erasure that someone addresses to you.

At the request of the person concerned, the Provider may erase what concerns
them. Section 3 of the
[privacy policy](/v1/public/legal/confidentialite?lang=en) sets out how.

## 9. Public surfaces

**Your Wall** is public only if you publish it, and shows nothing but what you
put there about yourself. You can unpublish it at any moment; it then becomes a
status page that reveals nothing about you.

**Links** give access to a single resource, never to a set, and you can revoke
them at any moment. Pages served by a link token are not indexed by search
engines.

**Contributions** — a gift idea, a note — are submitted to you: you decide to keep
or set aside each one. A contributor sees what became of what they suggested,
without comment from you.

**Birthday notes** left for you are for you alone. They are never displayed
publicly, and the Service keeps no guest book.

**A reservation** on a wish is never shown together with the name of whoever made
it, unless that person chose to be known.

## 10. Rules of use

You may not use the Service to keep watch on a person, to build a file on people
you have no personal connection with, to harass or to harm, to distribute
unlawful content, or to attempt to reach another account's data.

Links entered by third parties are shown in full and opened in isolation; the
Provider guarantees neither their content nor their safety.

Use contrary to these rules may lead to immediate suspension of the account.

## 11. Intellectual property

The Lehno name, its logotype, its graphic elements and its code are the property
of the Provider. Nothing in these Terms transfers any right to them.

## 12. Availability and liability

The Provider takes reasonable steps to keep the Service available, without
guaranteeing uninterrupted or error-free operation.

Automatically generated content is a **proposal**: it may be inaccurate or
clumsy, and it is for you to read it before using it. The Provider cannot be held
liable for the use you make of content you approved and sent.

The Provider is not liable for a reminder that fails to arrive through a third
party — a carrier, an email provider, a device setting — nor for the consequences
of a date you entered incorrectly.

## 13. Suspension and deletion

You can delete your account at any time from the app. The flow first tells you what
disappears, offers to refund the credits you bought (section 6), then asks you to
confirm with a one-time code.

Once confirmed, the account is **deactivated**: signing in is no longer possible,
your public surfaces stop responding and reminders stop. Data is kept for **thirty
days**, then permanently erased — stored files included. During that period, you can
still turn back by writing to support.

The Provider may suspend an account for breach of these Terms, after informing
the User except where seriousness or urgency prevents it.

## 14. Changes

These Terms may change. Any substantial change is notified at least fifteen (15)
days before it takes effect and requires fresh acceptance at the next sign-in.
Each acceptance is timestamped and kept.

## 15. Governing law and disputes

These Terms are governed by **Cameroonian law**. In the event of a disagreement,
an amicable solution is sought by writing to hello@lehno.app before any legal
action. Failing agreement, the dispute falls to the **competent courts of
Cameroon**.

Mandatory consumer-protection provisions applying to residents of the European
Union remain in force.
