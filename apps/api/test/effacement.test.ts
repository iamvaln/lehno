...
describe("l'effacement des comptes supprimés", () => {
   ...
   *
   * Sans cette exclusion, le compte partirait avec son numéro, et la demande de
   * remboursement resterait en attente pour toujours — plus aucune coordonnée
   * où verser, et personne pour en ajouter. Le titulaire, lui, croit avoir
   * supprimé son compte. */
  describe("le remboursement dû retient l'effacement", () => {
    const avecRemboursement = async (statut: "pending_deletion" | "deleted") => {
      const u = await compte({
        status: statut,
        ...(statut === "pending_deletion"
          ? { deletionRequestedAt: new Date(Date.now() - 60 * 86_400_000) }
          : {}),
      });
      await db.prisma.payment.create({
        data: {
          userId: u.id, direction: "refund", mode: "manual", status: "pending",
    ...
    it("laisse en place un compte dont le remboursement est encore dû", async () => {
      const u = await avecRemboursement("pending_deletion");
      await service.executer();
      const apres = await db.prisma.user.findUniqueOrThrow({ where: { id: u.id } });
      expect(apres.erasedAt).toBeNull();
      expect(apres.email).not.toBeNull();
    });

    /* L'exclusion vaut aussi pour la porte de l'administration : forcer un
       effacement ne doit pas faire disparaître la coordonnée d'une dette. */
    it("retient aussi un effacement forcé par l'administration", async () => {
      const u = await avecRemboursement("deleted");
      await service.executer();
      expect((await db.prisma.user.findUniqueOrThrow({ where: { id: u.id } })).erasedAt).toBeNull();
    });

    // Réglé — versé ou annulé —, le compte repart dans la file.
    it("laisse partir le compte une fois le remboursement réglé", async () => {
      const u = await avecRemboursement("pending_deletion");
      await db.prisma.payment.updateMany({
        where: { userId: u.id, direction: "refund" }, data: { status: "succeeded" },
      });
      await service.executer();
      expect((await db.prisma.user.findUniqueOrThrow({ where: { id: u.id } })).erasedAt).not.toBeNull();
    });
  });
});
