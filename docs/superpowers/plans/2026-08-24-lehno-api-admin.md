...
- [ ] **5. Comptes** *(les entités, dans l'ordre demandé : utilisateurs, puis fiches)* — `GET /admin/users` (curseur, filtres), `GET`/`PATCH /admin/users/{id}`, `POST /admin/users/{id}/credits`, `/device-limit`. Le détail ne rend **ni fiches, ni notes, ni souhaits** : le cloisonnement tient en administration, et `compteDetailSchema` ne les porte pas.
